...
try {
  ...
  $stmt = $pdo->prepare("INSERT INTO {$tablePrefix}clicks_table (ip_address, ad_unit_id, fingerprint) VALUES (?, ?, ?)");
  $stmt->execute([$ip_address, $ad_unit_id, $fingerprint]);

  // Check if the IP has exceeded the click limit for the specific ad unit
  $stmt = $pdo->prepare("SELECT COUNT(*) as click_count FROM {$tablePrefix}clicks_table WHERE ip_address = ? AND ad_unit_id = ? AND clicked_at > NOW() - INTERVAL ? $timeUnit");
  ...
  if ($clicks['click_count'] >= $clickLimit) {
    ...
    $stmt = $pdo->prepare("SELECT 1 FROM {$tablePrefix}blocked_ips_table WHERE ip_address = ? AND ad_unit_id = ?");
    $stmt->execute([$ip_address, $ad_unit_id]);
    $exists = $stmt->fetch();

    if ($exists) {
      // Update the existing record for the specific ad unit
      $stmt = $pdo->prepare("UPDATE {$tablePrefix}blocked_ips_table SET block_until = NOW() + INTERVAL ? $blockUnit, fingerprint = ? WHERE ip_address = ? AND ad_unit_id = ?");
      $stmt->execute([$blockQuantity, $fingerprint, $ip_address, $ad_unit_id]);
    } else {
      // Insert a new record for the specific ad unit
      $stmt = $pdo->prepare("INSERT INTO {$tablePrefix}blocked_ips_table (ip_address, ad_unit_id, block_until, fingerprint) VALUES (?, ?, NOW() + INTERVAL ? $blockUnit, ?) ON DUPLICATE KEY UPDATE block_until = NOW() + INTERVAL ? $blockUnit, fingerprint = ?");
  ...
  if (isIPBlocked($pdo, $tablePrefix, $ip_address, $ad_unit_id)) {
